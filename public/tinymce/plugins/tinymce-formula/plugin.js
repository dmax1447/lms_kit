;(function (tinymce) {
  ...
  console.log(this.$nuxt.$i18n.messages[locale])
  const { modalTitle } =
    this.$nuxt.$i18n.messages[locale].tinymce.plugins.formula
  ...
  function buildIFrame(editor, url) {
    ...
    if (
      ...
      if (currentNode.getAttribute('data-equation'))
        equationParam = '&equation=' + currentNode.getAttribute('data-equation')
    }
    const html =
